...
const ProductPriceForm = ({ addPrice, updatePrice }) => {
  ...
  useEffect(() => {
    if (priceId) {
      const fetchPrice = async () => {
        try {
          const response = await axios.get(`http://localhost:3000/product_price/${priceId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (priceId) {
        await axios.put(`http://localhost:3000/product_price/${priceId}`, priceData);
        toast.success('Price updated successfully.');
        updatePrice(priceData);
      } else {
        await axios.post('http://localhost:3000/product_price', priceData);
