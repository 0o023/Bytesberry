...
function DeliveredProducts() {
  ...
  useEffect(() => {
    async function fetchOrders() {
      try {
        const response = await axios.get('http://localhost:5000/get_delivered_orders'); // Use the correct URL and port
        ...
        <div className="flex flex-col w-full p-10 box-border bg-beige">
          <div className="w-full flex flex-col items-center">
            {loading ? (
              ...
              <>
                <h1 className="text-4xl font-bold mb-2 text-[#333] mt-0 -mt-7">Delivered Products</h1>
                <div className="w-full mt-[10px]"> 
                  {orders.map((order, index) => (
                    <Link to={`/more-delivered/${order.order_id}`} key={index} className="no-underline">
                      <div 
