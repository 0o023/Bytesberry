// controllers/product_variety_sizeController.js
const pool = require('../db/pool');
const { addProductVarietySizeSchema } = require('../Schemas/product_variety_size.schema');

const addProductVarietySize = async (productId, sizeName) => {
    // Validate the inputs
    const { error } = addProductVarietySizeSchema.validate({
    ...
    try {
        const result = await pool.query(
            'SELECT add_product_variety_size($1, $2)',
            [productId, sizeName]
        );
